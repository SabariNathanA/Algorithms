...
public class SmallestKElementsUsingPartition {
	...
	public static void compute(float[] a,int twiceK) throws IOException{
		...
		try{
			...
			while((dis.available()>0) ){
				medianIndex = partition(a,0,val-1);
				if(medianIndex>((val-1)/2)){
					val= medianIndex + 1;
			...
			int temp=val-(twiceK/2);//because to find the first 10,000 numbers alone
