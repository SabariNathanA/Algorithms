...
/* Start of edge  inputs*/
	...
	private void initQueue(int n){
		...
		int i;PriorityVertex temp = new PriorityVertex();
		for(i=0;i<n;i++){
			temp = new PriorityVertex(i, 1000);
...
//		this.minHeap.add(temp);
		this.comparePriDecreased(startVertex, 0);
		...
		while(this.minHeap.size() !=0){
			...
			while(head != null){//
				v = head.vertex;
				if(this.comparePriDecreased(v, head.weight )){
					parentOfVertex[v] = u;
				}
				head = head.next;
			}
	...
	private boolean comparePriDecreased(int v, int edgeW_UV){
		int relaxationValue = edgeW_UV;
		boolean toret = false;
